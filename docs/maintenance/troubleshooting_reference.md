...
> [!NOTE] 
> **歴史的コンテキストと最新仕様 (SSoT 3.1)**
> 以下の手順書には、/home/heppo/ 等の当時の生々しい絶対パスや、初期アーキテクチャ（.jsonlファイルへのツール履歴書き込み等）の記録がそのまま保存されています。これらは「どのようにデバッグの壁を越えたか」を示す極めて重要な文脈（歴史）です。
> ※ 現在はメモリ内データ注入と**ゼロ幅文字（ZWC）を用いたステガノグラフィ通信（SSoT 3.1）**に移行したため、.jsonl ファイルを用いた監視などの一部手順は意味を持ちませんが、過去の不具合原因を理解するために一文字も消さずに残しています。コード変更時は必ず `AI_CRITICAL_CONTEXT.md` を熟読してください。
